...
글번호, 제목, 작성자<hr>
<%
String pageNum=request.getParameter("page"); //request객체에서 page피라미터 값 받아오기!
if(pageNum==null){
	pageNum="1";
}
Dao dao=new Dao();
...
String searchWord=request.getParameter("word"); //검색어 파라미터
...
for(int i=0;i<posts.size();i=i+1){ //각 게시글의 정보 출력
%>
<%=posts.get(i).no%>
...
<%
	...
	int nPageNum = Integer.parseInt(pageNum);	//페이지네이션 계산을 위해 변환.
	...
	boolean hasPrev = true;	//이전 블럭 가기 가능 여부 저장값 초기화 (이전 블럭이 있는가?)
...
<%				
	...
	for(int i=blockStartNo;i<=blockEndNo;i++){	// 	<< 이렇게 바꿈 (현재 블럭에서 시작~끝까지의 페이지 번호를 화면에 출력하기 위해)
