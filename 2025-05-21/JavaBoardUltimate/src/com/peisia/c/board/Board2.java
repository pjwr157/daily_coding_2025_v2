...
public class Board2 {
	...
	private ArrayList<Post2> posts = new ArrayList<Post2>();
	private int count = 0;	
	
	////	같은 패키지내의 클래스 Main에서 호출되므로 default 여도 충분하므로 그대로 둠
	void run() {
		System.out.println("간단 게시판 v0.0.6");	/* [버전0.0.6] */
		...
		while(true) {
			...
			case "3":	//쓰기
				...
				Post2 p = new Post2(count,title,con,writer);
				posts.add(p);
			...
			case "4": //삭제
				...
				for(int i=0;i<posts.size();i=i+1) {
					Post2 pp = posts.get(i);
					
					if(pp.getNo() == selectNoForDel) {	/* [버전0.0.6] */
			...
			case "5":
				...
				for(Post2 p3:posts) {	
					if(p3.getNo() == selectNo3) {	/* [버전0.0.6] */
						System.out.println("=================");
						System.out.println(p3.getContent());	/* [버전0.0.6] */
						System.out.println("=================");
						System.out.println("바꿀 내용입력하세요");
						String newContent = sc.next();
//						p3.content = newContent;
						p3.setContent(newContent);	/* [버전0.0.6] */
						break;
					}
				}				
				break;
