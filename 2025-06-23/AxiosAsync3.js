...
function App() {
    ...
    useEffect(() => {
        axios.get('http://localhost:8080/api/xw')
            .then(response => {
...
//{data != null ? data.as[0] : ''} 이거랑 이거 활용하기{data?.as?.[0] ?? ''} (Opitonal Chanin)
    return (
        <div>
            <h1>데이터 확인</h1>
            {error && <p>데이터를 불러오는 중 에러가 발생했습니다.</p>}
            {data ? <pre>{JSON.stringify(data, null, 2)}</pre> : <p>데이터를 불러오는 중...</p>}
            <p>고양이 이름: {data != null ? data.catName : ''}</p>
            <p>{data != null ? data.catName : ''} 의 무기: {data != null ? data.myWeapon.name : ''}</p>
            <p>{data != null ? data.catName : ''} 의 무기공격력: {data != null ? data.myWeapon.atk : ''}</p>
        </div>
    );
}
